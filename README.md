...
macOS 可直接双击项目目录中的 `Media.app`：仅在后台启动 4173 服务，不打开浏览器；服务已经运行时复用现有进程，不会重复启动，不需要保持终端窗口打开。

设置页 `/settings` 提供「开机启动 4173 后端」开关，切换后自动保存。开启后，下次登录当前 macOS 用户时直接启动 `app.py --host 127.0.0.1 --port 4173`，不打开浏览器；关闭不会停止当前服务。配置保存在 `~/Library/LaunchAgents/local.personal-ai.media.backend.plist`，移动项目后应先关闭开机启动，再在新位置重新开启。

构建应用：`python3 build_media_app.py`。应用依赖当前项目目录及 `.venv`，可将应用复制到「应用程序」目录；移动项目目录后需移走旧应用并重新构建。启动失败会显示提示，服务日志位于 `data/media/service.log`。此应用仅启动本地工作台，远程 ComfyUI 等服务仍沿用现有配置。
...
播放器的「影片资料」区展示完整片名、演员、发行商、类型、发行日期、资料来源与本地视频参数；可以按片名、演员或编号搜索。刷新片库会更新资料，不重新加载正在播放的视频。

资料保存在 `data/player/catalog.json`，按影片编号索引；网络资料须保存来源及检索日期，第三方记录标记为待核实。`pending` 表示尚未检索，`partial` 表示第三方资料待核实，`verified` 表示已核实，`not_found` 表示检索后资料不足。页面中的「已检索」计数包含资料不足项，不代表资料完整，当前没有后台自动网络采集任务。

执行 `.venv/bin/python collect_media_technical.py`，按片库修改时间顺序逐部采集时长、分辨率和音视频编码，写入 `data/player/technical.json`，不改动视频。每部完成后保存，可重复执行续采；文件大小或修改时间变更后旧参数不再显示。

封面只加载本地已审核的非露骨图片：放在 `data/player/covers/`，对应资料设置 `cover: {"file": "文件名.jpg", "reviewed_safe": true}`。缺失或未审核图片显示编号占位卡，不把占位图算作已收集封面。封面接口拒绝访问目录外路径。

访问 `/player` 使用本地片库；搜索、格式筛选和刷新不会中断当前视频，当前视频不在筛选结果中时暂停上下集和自动切集。

- 控制栏在播放中无操作满 **5 秒**后开始淡出，动画 **300 毫秒**；移动鼠标、触摸或按键唤醒，首次触摸唤醒不切换播放状态。暂停、拖动进度、展开菜单、键盘焦点位于控制栏时保持显示，全屏行为一致。
- 单击画面播放暂停，双击切换全屏；提供前后跳转 10 秒、缓冲进度与时间预览，以及顺序播放、单集循环、列表循环（按筛选结果循环）。倍速支持 0.5、0.75、1、1.25、1.5、2 倍。
- 音量、静音、倍速、画面适配和播放模式保存在当前浏览器；进度每 2 秒保存，暂停、切集及离开页面时补存，播放结束清除记录。续播提示支持从头播放；浏览器存储不可用时仍可播放，但无法持久保存。
...
输入框、按钮及菜单内保留原生键盘行为。画中画、全屏及复制需浏览器支持；不可用时入口禁用并显示原因，失败时提示重试。解码能力沿用浏览器，可下载原文件使用本地播放器打开。
...
- `IMAGE_GENERATION_BACKEND`：图片后端，`comfy`、`seetacloud` 或 `local`
- `LOCAL_IMAGE_API_URL` / `LOCAL_IMAGE_API_KEY` / `LOCAL_IMAGE_MODEL`：本地 Q4 OpenAI-compatible 生图服务配置
...
本地 Q4 服务使用 `./tools/qwen-image-q4/serve.sh` 监听 `127.0.0.1:6009`；设置 `IMAGE_GENERATION_BACKEND=local` 后，Photo Lab 会调用其 OpenAI-compatible `/v1/images/generations` 接口并把 `b64_json` 图片归档到现有任务历史、图库和标注流程。远端 SeetaCloud 仍可通过 `IMAGE_GENERATION_BACKEND=seetacloud` 切回。
